...
zcite.listStyles = function(filter){
	var jsonObj = {};
	jsonObj.styles = {};
	//zcite.debug(jsonObj);
	if(zcite && zcite.cslFetcher) {
		if(zcite.cslFetcher.cslShortNames) {
			for(var name in zcite.cslFetcher.cslShortNames) {
				if(zcite.cslFetcher.cslShortNames.hasOwnProperty(name)) {
					jsonObj.styles[name] = zcite.cslFetcher.processStyleIdentifier(name);
				}
			}
		}
		zcite.debug(jsonObj);
		if(zcite.cslFetcher.cslDependentShortNames){
			for(var name in zcite.cslFetcher.cslDependentShortNames) {
				if(zcite.cslFetcher.cslDependentShortNames.hasOwnProperty(name)) {
					jsonObj.styles[name] = zcite.cslFetcher.processStyleIdentifier(name);
				}
			}
		}
...
zcite.listStyles0 = function(filter){
	var jsonObj = {};
	jsonObj.styles = {};
	//zcite.debug(jsonObj);
	if(zcite && zcite.cslFetcher) {
		if(zcite.cslFetcher.cslShortNames) {
			jsonObj.cslShortNames = zcite.cslFetcher.cslShortNames;
		}
		zcite.debug(jsonObj);
		if(zcite.cslFetcher.cslDependentShortNames){
			jsonObj.cslDependentShortNames = zcite.cslFetcher.cslDependentShortNames;
		}
		zcite.debug(jsonObj);
	}
	return JSON.stringify(jsonObj);
...
http.createServer(function (request, response) {
    ...
    if(request.method == "OPTIONS"){
    	...
    	var jsonStr = '';
        zcite.debug("get request received", 5);
        var uriObj = url.parse(request.url);
        //zcite.debug('href == ' + uriObj.href);
        //zcite.debug('search == ' + uriObj.search);
        //zcite.debug('query == ' + uriObj.query);
        zcite.debug('pathname == ' + uriObj.pathname);
        if(uriObj.pathname == "/styles/") {
        	var filter = 'none';
        	if(uriObj.query) {
        		uriObj.parsedQuery = require('querystring').parse(uriObj.query);
        		if(uriObj.parsedQuery && uriObj.parsedQuery.filter){
        			filter = uriObj.parsedQuery.filter;
        		}
        	}
        	//zcite.debug('filter == ' + filter);
        	jsonStr = zcite.listStyles0(filter);
        	//zcite.debug('jsonStr == ' + jsonStr);
			var nowdate = new Date();
			response.writeHead(200, {
				'Date': nowdate.toUTCString(),
				'Allow': 'POST,GET,OPTIONS',
			...
			zcite.debug('jsonStr.length == ' + jsonStr.length);
			if(jsonStr.length > 0) {
				response.write(jsonStr, 'utf8');
			}
			response.end('');
        } else {
